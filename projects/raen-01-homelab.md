...
RAEN-01 is the foundational RACBCONSULTING homelab platform used to develop, validate, operate, and document infrastructure for internal systems, automation workloads, AI tooling, and technical experiments.
...
RAEN-01 was designed around a different operating principle:
...
RAEN-01 includes dedicated host-level observability through Netdata.
...
RAEN-01 intentionally allows selected native host-level AI tooling where native execution is technically justified.

Current verified runtime observations include:
...
The quarantined work is not presented as part of the active RAEN-01 runtime.
...
RAEN-01 is not presented as a highly available production cluster or as an externally certified security platform.

Current verified boundaries include:

- single primary homelab node;
- no HA claim;
...
**RUNTIME CORRELATION** — direct host validation confirms the current Docker/runtime platform, active core services, host observability, current Paseo runtime, and current Claude/Codex availability through the daemon.
...
RAEN-01 is an internal RACBCONSULTING infrastructure platform, not a public infrastructure template or open-source distribution.
